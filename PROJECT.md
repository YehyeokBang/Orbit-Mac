# PROJECT.md — Orbit-mac 프로젝트 규칙

이 파일은 Orbit 프로젝트 특수 규칙입니다. 일반 행동 가이드라인은 `CLAUDE.md` 참조.
...
- **destructive shell command 그냥 실행 금지.** `rm -rf`, `git reset --hard`, `git push --force` 등은 사용자에게 확인 후.
...
./deploy.sh
```
종료 → 복사 → 재실행 → 로그 확인 → 권한 필요 시 시스템 설정 자동 오픈까지 한 번에 처리.

**권한이 매번 리셋되는 근본 원인:**
빌드마다 바이너리 해시 변경 → ad-hoc 서명 해시 변경 → macOS TCC가 "다른 앱"으로 인식.
영구 해결: Xcode → Signing & Capabilities → Team에 Apple ID 연결 → TeamIdentifier 부여 → TCC가 bundle_id+team_id로 추적하여 이후 빌드에서도 권한 유지.

**tccutil reset은 사용하지 말 것.** 권한을 완전히 날림. 권한 문제 시 시스템 설정에서 토글만.
...
- 기본 브랜치: `main`. 변경 작업은 `feature/<작업명>` 브랜치에서 PR로 병합.
...
office-hours 결과물(premise 정리, approach A/B 비교, 검증 절차)은 `~/.gstack/projects/Orbit/`에도 저장됨. 다음 세션에서 컨텍스트 잃었으면 거기 참고.
