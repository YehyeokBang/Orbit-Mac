...
| `←` `→` `↑` `↓` | 방향 기반 이동 (가장 가까운 창) |
| `Enter` | 선택한 창으로 전환 |
| `1` ~ `9` | 해당 번호 창으로 바로 이동 (메뉴바에서 활성화 필요) |
| `Cmd+Delete` | 선택한 앱 종료 |

- **자동 포커스** — Mission Control을 열면 직전에 보던 창이 자동으로 선택됨
- **번호 배지** — MC 활성 시 각 thumbnail 위에 탭 순서 번호(1~9) 표시 (메뉴바에서 토글)
- **숫자 키 이동** — 번호를 보고 바로 해당 창으로 점프 (메뉴바에서 토글, 기본 off)
